...
class Handler(FileSystemEventHandler):
    ...
    def on_any_event(event):
        if event.is_directory:
            ...
            # Add nbviewer.org link to comment cell
            if value.endswith(".ipynb"):
                with open(event.src_path, "r+") as f:
                    content = f.read()
                    nbviewer_url = f"https://nbviewer.org/url/{os.environ['HOSTNAME']}/{value}"
                    if "nbviewer.org" not in content:
                        f.seek(0, 0)
                        f.write(f"<!--\nLink to nbviewer: {nbviewer_url}\n-->\n\n{content}")
